...
- 双仓库职责：GitHub 开发主仓库保留 commit、PR、GitHub PR/Actions 与 Pages；NJU GitLab 课程镜像已创建为 Private。Pipeline #312781 的 DinD 构建因共享 Runner 缺少 privileged 权限失败；Kaniko 修复对应的 Pipeline #312784 中 `unit-test` 已通过，但 `docker-build` 在拉取 `gcr.io` 镜像时因 `context deadline exceeded` 失败。当前改用 Docker Hub 上的 `moby/buildkit:rootless`，修复验证中；检查前改为 Public。
