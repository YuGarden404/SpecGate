...
- Agent 类型与选择：Claude Code v2.1.70 无法连接 `api.anthropic.com`，且用户没有可用 Anthropic 服务条件；OpenCode 的官方签名/校验 Windows x64 二进制在本机无法加载，且本机无可用 WSL distribution；Gemini CLI 0.50.0 可启动，但用户账户无 Gemini Code Assist 使用权限。最终使用 Gemini Web 进行隔离审查。
- 会话隔离：Gemini Web 使用全新独立会话，只上传 `SPEC.md` 与 `docs/superpowers/plans/2026-07-16-final-delivery-compliance.md`。
- 尝试任务：任务 2“同步当前发布版本与证据链”和任务 3“增加完整的直接依赖许可证表”。
- 暂停问题：Gemini Web 在任务 2 的步骤 1 和步骤 3、任务 3 的步骤 1 和步骤 4 因缺少当前完整文件内容而暂停，并请求 `tests/test_final_evidence.py`、`docs/FINAL_EVIDENCE_MATRIX.md`、`docs/FINAL_SUBMISSION_CHECKLIST.md`、`docs/REFLECTION_FACT_CHECK.md`、`PLAN.md`、`AGENT_LOG.md`、`README.md`。
- 草案与测试：Gemini Web 给出两个任务的骨架补丁草案，并明确没有本地 shell、没有可供任务使用的外部网络工具，也无法直接操作工作区文件；没有修改任何文件，没有运行任何测试，草案也未被当作已应用结果。
- 计划修订：增加 Web-only 审查环境与本地实现环境的执行前提；不再上传上述七个文件，缺少目标文件上下文作为能力边界证据保留，实际实现交由本地 Subagent。
- 人工参与：用户选择并启动工具、提供两个隔离输入文件、转交 Gemini Web 原始结果，并决定不再补充七个目标文件。Gemini Web 尝试与暂停过程耗时约 3 分钟。
- 历史边界：这是最终合规阶段的补充验证，不替代 2026-07-08 的早期审查，也不追溯性声称 MVP 实现前完成过完整实现试跑。

# 2026-07-16 最终交付合规修复

- 审查起点：PR #20 合并后的 `main@c39d101`；完整回归为 `Ran 908 tests in 210.559s`、`OK (skipped=27)`。这是 2026-07-16 审查起点证据，本阶段最终数字另行刷新。
- 最近已合并链：后端审计加固 `d3607c4` / PR #18 / `8d30ca5`；Web 真实 LLM 接入 `5279a7c` / PR #19 / `b98563a`；真实 LLM 生命周期修复 `e35eb46` / PR #20 / `c39d101`。
- 计划校准：`a25a4b6`（明确本地合规实施环境与 Web-only 冷启动边界）。
- 任务 1：`ff1f74a`（补充冷启动证据）、`89fef8d`（加强冷启动证据契约）。
- 任务 2：`5bbe548`（同步最终发布证据）、`8d66ab1`（收紧发布证据契约）。
- 任务 3：`f6d7b76`（记录直接依赖许可证）、`01cae8c`（验证许可证表）。
- 任务 4：`6380410`（如实记录 Open Design 流程偏离）。
- 任务 5：`6626f44`（区分静态评审与公网部署）、`41aa125`（精确约束交付状态表）。
- 任务 6：`3355033`（加入当前 PR/CI 归属证据）、`f05b10f`（强化远端证据验证）、`1de68fc`（拒绝部署声明变体）、`3438457`（规范公网部署声明）、`57cc069`（处理否定 GHCR 发布声明）。
- 任务 7：当前最终验证为 `Ran 919 tests in 402.898s`、`OK (skipped=27)`，退出码 0；六项确定性机制为 `Ran 6 tests in 47.709s`、`OK`。冻结提交为 `ed3572e`，质量修复为 `f0c3bf0`；完整 SHA 可从当前任务 Git 历史核对。
- 语法与安全：Python 编译、JavaScript 语法、Git 空白检查均退出码 0；`.env` 被忽略且提交历史为空，疑似密钥模式扫描无命中。
- Pages 边界：主线程本轮只读复核首页、demo、report 均正常加载并显示预期标题与主标题；本地验证 Subagent 没有亲自浏览远端。公网交互式 Web 后端与公开容器 registry 仍待后续阶段。
- 全阶段终审修复：已将反思事实清单纳入当前远端证据契约，并补齐部署声明跨主体否定作用域的永久 mutation；任务 1–7 的完成勾选与冻结证据保持不变，最终修复提交 SHA 在提交后由 Git 历史核对。

# 2026-07-17 最终提交同步与双仓库交付

- 当前快照：PR #23 已合并到 `main@5fd86fa`；NJU SE Hub 审计分支完整回归为 `Ran 921 tests in 403.030s`、`OK (skipped=27)`。
- GitHub 证据：[CI #59](https://github.com/YuGarden404/SpecGate/actions/runs/29566219258) 与 [Pages #34](https://github.com/YuGarden404/SpecGate/actions/runs/29566219221) 均成功；列表截图 `docs/evidence/github-actions-pr23-final.png` 和两张 job 详情截图已归档。PR #20 的 CI #53、Pages #31 与旧截图继续作为历史证据。
- 产品与双仓库职责：SpecGate 是 CLI-first Harness，WebUI 是配套评审入口。GitHub 开发主仓库保留 commit、PR、完整 GitHub Actions、Docker 构建与 Pages；NJU GitLab 课程镜像已创建为 Private。Pipeline #312781、#312784、#312797 的三次 `unit-test` 已通过，容器构建分别受 DinD、GCR 网络和 RootlessKit 权限限制；GitLab CI 随后只保留 `unit-test`，Pipeline #312806 在 `main@66ea825` 上通过，检查前改为 Public。
- 同步边界：首次 GitLab 同步只推送 `main` 与 tags，不推送旧本地功能分支；项目创建、remote、push、可见性切换和截图均由用户执行。
- 部署边界：公网交互式 Web 后端与公开容器 registry 继续等待教师答复，不写成已完成。

# 2026-07-18 CLI 易用性与 GHCR 公开镜像分发

- 产品边界：核心继续是 `specgate` CLI 与自行实现的 Harness；WebUI 保留为显式启动的配套评审入口，不部署公网服务。
- 用户路径：新增只保存非敏感字段的用户配置，`specgate configure` 隐藏输入 API key，日常运行收敛为 `specgate run <工作区>`；解析顺序为命令行、环境变量、用户配置，凭据仍为环境变量优先于 keyring。
- 容器路径：Docker 默认入口改为 CLI；常规 GitHub CI 验证 CLI help、Mock Demo 与显式 WebUI help。
- 发布路径：`.github/workflows/ghcr.yml` 校验 `pyproject.toml` 版本与 `v0.1.0` 标签，计划发布 `0.1.0`、`0.1`、`latest` 与 commit SHA 标签，记录 OCI revision 与 digest。
- 发布结果：PR #25 已合并到 `main@44b236f`，分支最终验证为 `Ran 947 tests in 227.115s`、`OK (skipped=27)`；[CI #63](https://github.com/YuGarden404/SpecGate/actions/runs/29649068245)、[Pages #36](https://github.com/YuGarden404/SpecGate/actions/runs/29649068246) 与 [GHCR #1](https://github.com/YuGarden404/SpecGate/actions/runs/29649149933) 均成功。
- 公开分发：GHCR 公开镜像已完成匿名拉取验证，`ghcr.io/yugarden404/specgate:0.1.0` 的 digest 为 `sha256:324fad1d8ae82880990a3e032847408b9339bf52bd81dc53b61e74dcb4b6ea3d`。当前证据包为 `docs/evidence/github-actions-pr25-ci-success.png`、`docs/evidence/github-actions-pr25-pages-success.png`、`docs/evidence/github-actions-ghcr-v0.1.0-success.png`、`docs/evidence/github-package-specgate-public.png` 与 `docs/evidence/ghcr-anonymous-pull-smoke.png`。
- 部署边界：公开容器 registry 已完成；公网交互式 Web 后端未部署，发布镜像不等于部署服务。
- 学生边界：学生先自行修改 `REFLECTION.md`，再明确批准 AI 只做中等强度润色和结构整理；观点与案例保持学生原意。当前机械检查为 2430 个非空白字符，且已移除过期的“未来 provider”表述。

# 2026-07-19 最终使用文档与 v0.1.1 发布准备

- 工作方式：在隔离工作树 `final-docs-v011-prep` 上按 Superpowers Inline Execution 执行；设计和中文实施计划分别位于 `docs/superpowers/specs/2026-07-19-final-docs-v011-release-design.md` 与 `docs/superpowers/plans/2026-07-19-final-docs-v011-release.md`。所有 Git add、commit、push、tag 和 PR 操作仍由用户执行。
- 两阶段边界：阶段 A 只把包版本、测试契约、README、部署说明、讲解稿、证据材料和过程记录准备为 `v0.1.1`；阶段 B 必须等待阶段 A 合并后再创建标签，并以 GHCR workflow、Public Package、匿名 smoke、不可变 digest 和 OCI revision 作为发布门禁。`v0.1.0` 的标签、digest 和五张历史截图保持不变。
- 影响文件：`pyproject.toml`、`src/specgate/__init__.py`、`tests/test_imports.py`、`tests/test_final_evidence.py`、`README.md`、`docs/DEPLOYMENT.md`、`docs/PROJECT_WALKTHROUGH.md`、`docs/FINAL_EVIDENCE_MATRIX.md`、`docs/FINAL_SUBMISSION_CHECKLIST.md`、`docs/REFLECTION_FACT_CHECK.md`、`PLAN.md` 与 `AGENT_LOG.md`。不修改学生拥有的 `REFLECTION.md`。
- TDD 顺序：版本契约先从 `0.1.0 != 0.1.1` 得到 RED，再同步两处版本元数据得到 GREEN；最终证据契约先要求 PR #27、教师基线、干净安装、真实模型 smoke 和阶段 A/B 边界，27 项测试出现 11 个预期文档失败，再逐份更新材料。
- 教师源码基线：PR #27 合并后的 `main@6dbaa75` 在公开 NJU 仓库空目录克隆中得到 `Ran 954 tests in 213.679s`、`OK (skipped=27)`，退出码 0；远端 CI #67、Pages #38、Pipeline #313088 与 job #596503 均成功。
- 演示事实：固定 Mock Demo 退出码 0、Gate 通过且 trust 为 `trusted`；真实 CLI smoke 使用 `glm-5.2`，得到 `passed=True, steps=2`、最终 Gate 通过、`parse_errors=0`，随后清除系统 keyring 凭据。
- 阶段 A 验收：安装与工作区命令可以从 README 直接复现；部署文档必须区分源码构建、历史 `v0.1.0` 和尚未发布的 `v0.1.1`；最终证据契约、聚焦测试、Python 编译、JavaScript 语法、密钥扫描、完整测试和分支范围审查全部通过后，才允许创建 PR。
- 阶段 A 发布准备分支验证：imports 1 项、CLI 51 项、workflow 5 项、最终证据 27 项均通过；Python 编译、JavaScript 语法和疑似真实密钥模式扫描通过；完整套件得到 `Ran 954 tests in 418.617s`、`OK (skipped=27)`，退出码 0。该结果与教师 `Ran 954 tests in 213.679s` 基线分开保存。
- 阶段 A 发布准备分支验证在同步证据后独立复跑为 `Ran 954 tests in 417.907s`、`OK (skipped=27)`，退出码 0；两次完整运行数量和跳过数一致，耗时分别保留。
- 阶段 B 门禁：`v0.1.1` 标签必须指向阶段 A merge commit；匿名 Docker 配置中的 pull、CLI help、Mock Demo 与 Web help 均须退出码 0，RepoDigest 与 OCI revision 必须可核对。远端事实成立后另开证据同步分支，不预写 `0.1`、`latest`、digest 或“已发布”结论。
- 历史记录：旧计划复选框、PR #25/GHCR #1 证据、`v0.1.0` digest `sha256:324fad1d8ae82880990a3e032847408b9339bf52bd81dc53b61e74dcb4b6ea3d` 和早期失败—修复过程均保留，不用当前章节反向改写。

# 2026-07-19 v0.1.1 发布证据同步

- 实施入口：独立中文计划位于 `docs/superpowers/plans/2026-07-19-v011-evidence-sync.md`，在 `v011-evidence-sync` 隔离工作树按 Superpowers Inline Execution 执行。所有 Git add、commit、push、PR、合并和标签操作继续由用户执行。
- 当前源码链：PR #28 合并后的 `main@9cf9093` 由 CI #69 / run 29678498485、Pages #39 / run 29678498457 与 NJU Pipeline #313118 / job #596642 覆盖并通过；教师干净克隆的 PR #27 / `main@6dbaa75` / `Ran 954 tests in 213.679s` 基线单独保留，不被当前分支耗时覆盖。
- 标签链：`v0.1.1` 已发布；GitHub 与 NJU 的 annotated tag object 均为 `adb74ca0586b20e3cb5e32767bb409370e70c2ef`，peeled commit 均为 `9cf909341cd1a5feb8ed2b244ce31f0495016c4c`。该标签冻结在 Stage A merge commit，不随 Stage B 证据 PR 移动或重建。
