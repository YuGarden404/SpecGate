...
自动测试使用 Fake/Stub Resolver 与 Transport，不访问真实 DNS、socket 或 Provider。GitHub Pages 只是静态评审入口；真实模型能力需要运行带持久化数据库、凭据主密钥和网络策略的 Web 后端。
...
GitHub Pages 提供无需登录的静态首页、demo 和一次运行报告，与 Docker/服务器启动的交互式 Web 产品壳相互独立。静态报告会展示：
