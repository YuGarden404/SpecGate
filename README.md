...
GHCR Package 已设为 Public。v0.1.0 是已验证的历史公开镜像，CLI 用户可以直接使用：
...
源码版本 `0.1.1` 已完成阶段 A 发布准备，但 `v0.1.1` 尚未发布。只有标签 workflow、公开 Package、匿名 smoke、不可变 digest 与 OCI revision 全部核对后，才能把它写成已发布镜像；不得用 `0.1`、`latest` 或预期 digest 代替证据。`--env-file` 由 Docker 读取，应放在仓库外且不得提交；SpecGate 本身仍不读取 `.env`。公网交互式 Web 后端未部署，发布镜像不等于部署服务。
...
GitHub 是开发主仓库；`.github/workflows/ci.yml` 运行完整测试并执行 Docker 镜像构建检查，`.github/workflows/pages.yml` 发布静态评审入口，`.github/workflows/ghcr.yml` 校验版本标签并发布 CLI-first 镜像。当前源码基线是 PR #27 合并后的 `main@6dbaa75`，CI #67 与 Pages #38 均成功。历史发布链继续保留：PR #25 合并后的 [CI #63](https://github.com/YuGarden404/SpecGate/actions/runs/29649068245)、[Pages #36](https://github.com/YuGarden404/SpecGate/actions/runs/29649068246) 与 `v0.1.0` 触发的 [GHCR #1](https://github.com/YuGarden404/SpecGate/actions/runs/29649149933) 均成功；截图见 `docs/evidence/github-actions-pr25-ci-success.png`、`docs/evidence/github-actions-pr25-pages-success.png` 和三张 GHCR 证据图。
