...
仓库交付采用双仓库分工：GitHub 开发主仓库保留完整 commit、PR、GitHub PR/Actions 与 Pages 证据；[NJU GitLab 课程镜像](https://git.nju.edu.cn/YuyuanLiang/specgate) 已创建为 Private 并同步 `main@5fd86fa`，检查前改为 Public。初始 Pipeline #312781 的 `unit-test` 已通过，`docker-build` 失败，当前处于 daemonless 构建修复验证中。GitHub 平台的 PR/Actions 元数据不会迁移到 GitLab。
...
- Docker 本地与 GitHub CI 构建、GitLab CI 配置、GitHub Pages 和公开静态评审入口；NJU GitLab 项目已创建，初始 Pipeline 的 Python 测试通过，daemonless Docker 构建修复验证中；公网后端与公开 registry 尚未完成。
...
NJU GitLab 初始 Pipeline #312781 在 `main@5fd86fa` 上运行：`unit-test` 已通过，`docker-build` 失败。日志证明学校共享 runner 未启用 privileged 模式，`docker:26-dind` 无法启动 Docker daemon；仓库已改为使用 Kaniko 的 daemonless `--no-push` 构建，状态为修复验证中，在新 Pipeline 通过前不声明 GitLab Pipeline 成功。
