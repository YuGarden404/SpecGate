...
真实 OpenAI-compatible provider 首次使用时执行：

```powershell
specgate configure
```

Base URL 与 Model 保存到当前用户配置，API key 通过隐藏输入保存到操作系统 keyring，不写入用户配置文件。之后每个单目录工作区只需包含 `TASK_SPEC.md`、`CHECKLIST.md` 和可选的现有 `index.html`，日常命令收敛为：

```powershell
specgate run <工作区>
```

运行前会检查工作区及两个必需文件。运行成功后会输出最终 `index.html`、report 和 trace 的位置。Model 与 Base URL 的优先级为命令行参数、`SPECGATE_LLM_MODEL` / `SPECGATE_LLM_BASE_URL`、用户配置；API key 仍按 `OPENAI_COMPATIBLE_API_KEY`、操作系统 keyring 的顺序读取。
...
```powershell
specgate configure
specgate run examples/knowledge_nav
```
...
GHCR Package 已设为 Public，CLI 用户可以直接使用：
...
GHCR 公开镜像已完成匿名拉取验证。公开镜像为 `ghcr.io/yugarden404/specgate:0.1.0`，[Package 页面](https://github.com/YuGarden404/SpecGate/pkgs/container/specgate) 显示 Public，RepoDigest 为 `sha256:324fad1d8ae82880990a3e032847408b9339bf52bd81dc53b61e74dcb4b6ea3d`。匿名环境中的 pull、CLI help 和 Mock Demo 均以退出码 0 完成；证据见 `docs/evidence/github-actions-ghcr-v0.1.0-success.png`、`docs/evidence/github-package-specgate-public.png` 与 `docs/evidence/ghcr-anonymous-pull-smoke.png`。`--env-file` 由 Docker 读取，应放在仓库外且不得提交；SpecGate 本身仍不读取 `.env`。公网交互式 Web 后端未部署，不能由公开镜像或一次 CI build 推定为已经上线。
...
GitHub 是开发主仓库；`.github/workflows/ci.yml` 运行完整测试并执行 Docker 镜像构建检查，`.github/workflows/pages.yml` 发布静态评审入口，`.github/workflows/ghcr.yml` 校验版本标签并发布 CLI-first 镜像。PR #25 合并后的 [CI #63](https://github.com/YuGarden404/SpecGate/actions/runs/29649068245)、[Pages #36](https://github.com/YuGarden404/SpecGate/actions/runs/29649068246) 与版本标签触发的 [GHCR #1](https://github.com/YuGarden404/SpecGate/actions/runs/29649149933) 均成功；CI 与 Pages 详情截图见 `docs/evidence/github-actions-pr25-ci-success.png` 和 `docs/evidence/github-actions-pr25-pages-success.png`。
