...
- Docker 本地与 GitHub Actions 构建、GitLab `unit-test`、GitHub Pages 和公开静态评审入口；NJU GitLab unit-test-only Pipeline 已通过；GHCR 发布工作流已实现，远端公开性待验证；公网后端仍未部署。
...
GHCR 发布完成并设为 Public 后，CLI 用户使用：
...
仓库当前是“GHCR 发布工作流已实现，远端公开性待验证”；在版本标签 workflow、Package Public 设置和匿名 pull 全部成功前，公开容器 registry 仍记为待完成。`--env-file` 由 Docker 读取，应放在仓库外且不得提交；SpecGate 本身仍不读取 `.env`。公网交互式 Web 后端仍未部署，不能由公开镜像或一次 CI build 推定为已经上线。
...
GitHub 是开发主仓库；`.github/workflows/ci.yml` 运行完整测试并执行 Docker 镜像构建检查，`.github/workflows/pages.yml` 发布静态评审入口，`.github/workflows/ghcr.yml` 校验版本标签并发布 CLI-first 镜像。容器构建与 GHCR 发布的权威 CI 证据来自 GitHub Actions；当前发布工作流已实现，远端公开性待版本标签和匿名拉取验证。
