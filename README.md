...
仓库交付采用双仓库分工：GitHub 开发主仓库保留完整 commit、PR、GitHub PR/Actions 与 Pages 证据；[NJU GitLab 课程镜像](https://git.nju.edu.cn/YuyuanLiang/specgate) 已创建为 Private，检查前改为 Public。初始 Pipeline #312781 在 `main@5fd86fa` 上因共享 Runner 不支持 privileged Docker-in-Docker 而失败；改用 Kaniko 后，Pipeline #312784 的 `unit-test` 已通过，`docker-build` 又因访问 `gcr.io` 时出现 `context deadline exceeded` 而失败。当前 `.gitlab-ci.yml` 已改用 Docker Hub 上的 `moby/buildkit:rootless`，状态仍为修复验证中。GitHub 平台的 PR/Actions 元数据不会迁移到 GitLab。
...
- Docker 本地与 GitHub CI 构建、GitLab CI 配置、GitHub Pages 和公开静态评审入口；NJU GitLab 项目已创建，两次真实 Pipeline 的 Python 测试均通过，rootless BuildKit 构建修复验证中；公网后端与公开 registry 尚未完成。
...
除 CLI 和静态报告外，SpecGate 现在提供一个 mock-first 的本地交互式 WebUI 产品壳，用于演示“用户上传/创建项目 -> 和 harness 对话 -> 生成或修复 HTML -> 查看报告与审批”的完整流程。它不是直接修改用户原始目录，而是把项目导入到隔离工作区，再返回生成产物和报告。当前仓库已完成本地/Docker 启动路径与确定性测试，但没有把该 Web 后端部署为公网服务。
...
上述命令只验证本地交互式 WebUI。公网交互式 Web 后端与公开容器 registry 均未在本阶段部署或发布；服务器部署和 GHCR 分发保留为后续独立阶段，不能由 `Dockerfile` 或一次 CI build 推定为已经上线。发布镜像不等于部署服务。
...
当前开发来源是 GitHub 开发主仓库。NJU GitLab 课程镜像创建后会从同一 `main` commit 独立运行 `.gitlab-ci.yml`；GitLab Pipeline 的成功必须由实际 Pipeline 证明，不能用 GitHub Actions 成功替代。

NJU GitLab 初始 Pipeline #312781 在 `main@5fd86fa` 上运行：`unit-test` 已通过，`docker-build` 失败。日志证明学校共享 Runner 未启用 privileged 模式，`docker:26-dind` 无法启动 Docker daemon。随后推送的 Kaniko 修复触发 Pipeline #312784：`unit-test` 已通过，但 `docker-build` 在执行脚本前拉取 `gcr.io/kaniko-project/executor` 时出现 `context deadline exceeded`。这次失败定位在学校 Runner 到 `gcr.io` 的网络边界，不是 Dockerfile 或 Kaniko 构建命令失败。当前配置改用 Docker Hub 上的 `moby/buildkit:rootless` 执行 daemonless OCI 构建；新 Pipeline 通过前，GitLab 状态仍为修复验证中。
