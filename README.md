...
仓库交付采用双仓库分工：GitHub 开发主仓库保留完整 commit、PR、GitHub PR/Actions 与 Pages 证据；NJU GitLab 课程镜像尚未创建，创建后仅同步 `main` 与 tags，先保持 Private，检查前改为 Public，并独立运行 GitLab Pipeline。GitHub 平台的 PR/Actions 元数据不会迁移到 GitLab。
...
- Docker 本地与 CI 构建、GitLab CI 配置、GitHub CI/Pages 和公开静态评审入口；NJU GitLab 项目与 Pipeline 尚未创建和运行，公网后端与公开 registry 尚未完成。
