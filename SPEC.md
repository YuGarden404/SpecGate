...
SpecGate 是 AI4SE 期末项目的 A 类选题：一个 CLI-first 的 `Coding Agent Harness`。`specgate` CLI 与自行实现的 Harness 内核是核心产品；WebUI 是课程要求的配套评审与演示入口，不替代 Agent loop、工具、治理或 Gate。本项目要从零实现一个小型 coding agent harness，而不是配置现成 agent 框架。
...
- CLI-first 的 Python harness，以 `specgate` 为主要运行入口。
- 自己实现 agent 主循环。
...
### 2.3 交付材料 Agent 执行环境边界

Web-only Agent 只负责隔离上下文后的计划审查和补丁草案，不被视为已经修改文件或运行测试。本地文件修改和测试要求执行 Agent 具备独立 worktree、文件系统、shell、Python、Node.js 与 Git，并由本地 Agent/Subagent 执行。本节只说明交付材料的协作与验证环境，不改变产品需求或任何生产行为。
...
7. 作为真实 LLM 用户，我可以在 Web 设置页安全配置 API key、Base URL 和 Model，让新 run 使用真实模型创建或修改 `index.html`，同时避免密钥进入源码、日志或 Git 历史。
...
- 课程自动验收和默认 demo 使用 `MockLLM`，不需要凭据或网络；WebUI 仅在用户完整配置后为新 run 启用真实模型。
...
- 真实模式只允许精确白名单中的公网 HTTPS 主机，DNS 校验后固定 IP 与原始 SNI/Host，禁止重定向和系统代理，并限制超时、重试与响应体大小。
...
交互式 Web 产品壳是 CLI-first Harness 的配套入口，不是另一套 Agent 内核。它使用 `WebRuntimeCoordinator` 的固定 worker 和有界队列。SQLite schema v5 保存用户 Settings，以及每个 run 的 `runtime_config_json` 与 `llm_config_json` 两份不可变配置快照；首次执行、HITL resume 和 queued 重启恢复只使用创建时快照。默认使用 MockLLM；当 API key、Base URL 和 Model 完整时，新 run 通过 OpenAI-compatible Chat Completions 使用真实模型，Provider 失败不会降级到 Mock。取消、超时、发布和恢复使用显式状态转换，Debug/Audit 只展示脱敏后的实际生效配置。

GitHub Pages 仍提供静态首页、demo 和报告，作为无需登录、无需服务器进程的公开静态评审入口；它与本地交互式 Web 产品壳是两个不同入口。当前交付状态不包含公网交互式 Web 后端，后者留到合规完成后的独立部署阶段。
...
Web run 的 schema v5 配置模型包含两类快照。七项 Runner 字段规范化为 `runtime_config_json`；模型模式、规范化 Base URL、Model 与不可逆凭据 fingerprint 规范化为 `llm_config_json`。后续 Settings 或凭据修改不得改变已有 run；凭据更新、重存、清除或主密钥变化会让旧真实 run fail closed。

## 9. 凭据与分发设计

### 9.1 凭据

Mock 模式不需要 key。CLI 的 `specgate credentials status/set/clear <provider>` 使用操作系统 keyring；进程环境变量是只读、最高优先级的临时来源。Web 使用独立主密钥和 AES-256-GCM 保存 `openai-compatible` API key，支持录入、更新和清除且永不回显。完整配置只对新 run 启用真实模型；每次调用前均核对 run 冻结的 fingerprint，任何不匹配都失败关闭。
...
GitHub Pages 发布静态首页、知识图谱 demo 和运行报告，README 提供三个公开静态 URL；静态 Pages 不保存凭据，也不能调用真实 Provider。交互式 Web 产品壳可由 Docker 在本地启动，支持项目导入、默认 MockLLM 或可选真实模型 run、HITL、取消、Debug/Audit 和产物下载；生成 HTML 不在同源认证上下文中直接执行。公网交互式 Web 后端尚未部署，不能把静态 Pages URL 当作该后端地址。
...
- LLM：默认 `MockLLM`；可选 OpenAI-compatible Chat Completions adapter 只提供 Action 决策，Harness 机制仍由本地代码实现。
- 报告：Python 生成静态 HTML。
- 分发：Docker。
...
- CI 支持 Docker 本地与 CI 构建路径；不据此声称公开 registry 已完成。
- README 说明安装、运行、Docker 本地构建、凭据安全、已知限制、静态 Web 报告 URL。
- 源码、配置、日志、报告和 Git 历史中不能出现真实凭据。
- Web 使用固定 worker、有界队列，并对取消、超时、重启恢复和发布竞争进行确定性测试。
- schema v5 的 `runtime_config_json` 与 `llm_config_json` 快照对首次执行、resume 和重启恢复保持一致。
- 真实模式只访问精确白名单中的公网 HTTPS 主机，固定已验证 DNS IP，禁止重定向、系统代理和无界响应；失败不会降级到 Mock。
- 自动测试全部使用 Fake/Stub Transport，不访问真实 Provider、DNS 或 socket。
