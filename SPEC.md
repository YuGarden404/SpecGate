...
GitHub Pages 发布静态首页、知识图谱 demo 和运行报告，README 提供三个公开 URL；静态 Pages 不保存凭据，也不能调用真实 Provider。交互式 Web 产品壳由 Docker/服务器启动，支持项目导入、默认 MockLLM 或可选真实模型 run、HITL、取消、Debug/Audit 和产物下载；生成 HTML 不在同源认证上下文中直接执行。
