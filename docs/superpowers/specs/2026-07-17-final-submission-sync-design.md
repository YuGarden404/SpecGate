...
本地继续保留 `origin` 指向 GitHub，并新增 `nju` remote 指向用户在 `git.nju.edu.cn` 创建的空项目。首次同步使用现有分支与 tags；后续固定采用“GitHub PR 合并 → 更新本地 `main` → 推送 `main` 与 tags 到 NJU GitLab → 核对两边最终 commit”的流程。
...
- NJU GitLab Pipeline 截图：只在 Pipeline 实际通过且用户提供合格截图后加入。
